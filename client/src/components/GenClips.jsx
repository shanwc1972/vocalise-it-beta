...
const GenClips = () => {
  ...
  const translateText = async (myText) => {
    ...
    const url = `https://translation.googleapis.com/language/translate/v2?key=${API_KEY}&source=${sourceLang}&target=${targetLang}&q=${encodeURIComponent(
      myText
    )}`;
  ...
  const readText = async () => {
    ...
    if (!finalText) {
      toast.warning("Please enter text to be read or translated.");
      return;
    }
  ...
  const TTSsave = async () => {
    ...
    if (!Auth.loggedIn()) {
      toast.warning("Please log in to save the audio file.");
      return;
    }
    ...
    if (voiceData) {
      const base64Audio = btoa(
        new Uint8Array(voiceData).reduce(
      ...
      try {
        ...
        const { data: audioData } = await saveAudio({
          variables: { audioData: base64Audio },
        });
          ...
          audioContext
            ...
            .then(async (decodedData) => {
              ...
              const { data } = await saveClip({
                variables: { input },
                refetchQueries: [
                  { query: QUERY_GETCLIPS, variables: { username } },
                ],
              });
  ...
  return (
    <div className="my-10 px-10 md:my-20 md:px-20">
      <main className="border rounded-xl p-4 bg-gray-900 max-w-[1200px] mx-auto">
        <h1 className="mt-6 mb-12 text-3xl">VocaliseIt</h1>
        <section className="container">
          <div className="flex flex-col mb-4 items-start w-36">
          ...
          <details>
            ...
            <div className="flex">
              <div className="flex-row w-1/2 mr-2">
              ...
              <div className="flex-row w-1/2 ml-2">
